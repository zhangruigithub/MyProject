#_author_:王丽
#_editor_:张三香 2016.10.08


Feature:在售商品管理列表查询
	"""
		（1）商品价格、商品库存和商品销量的查询支持只输入最低值或只输入最高值；
		（2）商品库存只输入库存最低值时，能把库存为'无限'的商品查询出来
		（3）库存查询，多规格商品只要有一个规格的库存满足查询条件，就可以查询出来此商品
		（4）销量查询，多规格商品按照各规格商品销量的和去匹配查询条件，进行查询
		(5)数据准备：
...
Scenario:1 在售商品管理列表'默认'（空）查询
	#默认查询
	Given jobs登录系统
	When jobs设置在售商品管理列表查询条件
		"""
		{}
		"""
